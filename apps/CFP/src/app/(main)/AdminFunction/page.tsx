...
export default function AdminFunctionPage() {
  ...
  const handleDelete = async (id: number | string) => {
    if (await confirm('確定要刪除此功能嗎？')) {
      try {
        await fetch(`${API_MAP.ADMIN_FUNCTION_MST}/Delete?id=${id}`, { method: 'POST' });
      ...
      <WrapContent className="p-3">
        ...
        <Container fluid className="mb-3">
            <div className="d-flex justify-content-end gap-2">
                <Btn color="success" icon="add" outline={false} onClick={() => router.push('/AdminFunction/Create')}>新增</Btn>
            </div>
        </Container>
