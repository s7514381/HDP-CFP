...
import { Toast } from "@/components/bootstrap5/Toasts";
...
/** 供應商平台網址 */
const VND_URL = process.env.NEXT_PUBLIC_VENDOR_URL;
...
export const ApiError = ({ apiEvent, onClose}: {apiEvent?: ApiEvent | null, onClose?: () => void}) => {
  ...
  const logoutUser = async () => {
    ...
    await api.post(API_MAP.LOGOUT);
    // 轉跳到供應商平台的登出action，進行前端的token清除與轉跳到登入頁面
    router.push(`${VND_URL}/site/logout`);
  }

  // 如果是401未授權，直接登出用戶
  useEffect(() => {
    if(apiEvent?.status === 401){
      logoutUser();
    }
  }, [apiEvent?.status]);
